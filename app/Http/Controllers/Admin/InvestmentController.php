...
class InvestmentController extends Controller
{
    ...
    public function newInvestUser(Request $request)
    {
        ...
        if ($request['acct_type'] == 'offshore') {
            ...
            if ((float) $request['amount'] > $withdrawable) return back()->with('error', 'Insufficient Funds in your HYSA, try again');
        } else {
            // $withdrawable = $ira;
    ...
    public function addRoi(Request $request, $investment_id)
    {
        ...
        if ($request['investment'] != $inv_amount) {
            ...
            if ($acct_type == 'offshore') {
                ...
                if ((float) $request['investment'] > $portfolioValue) return back()->with('error', 'Insufficient Funds in your HYSA, try again');
            } else {
                $portfolioValue = $ira;
